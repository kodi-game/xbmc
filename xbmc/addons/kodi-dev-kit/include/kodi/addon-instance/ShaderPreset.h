...
extern "C"
{
  ...
  typedef struct KodiToAddonFuncTable_ShaderPreset
  {
    ...
    preset_file(__cdecl* preset_file_new)(const AddonInstance_ShaderPreset* addonInstance,
                                          const char* path);
    void(__cdecl* preset_file_free)(const AddonInstance_ShaderPreset* addonInstance,
                                    preset_file file);

    bool(__cdecl* video_shader_read)(const AddonInstance_ShaderPreset* addonInstance,
                                     preset_file file,
                                     video_shader* shader);
    void(__cdecl* video_shader_write)(const AddonInstance_ShaderPreset* addonInstance,
                                      preset_file file,
                                      const video_shader* shader);
    // void (__cdecl* video_shader_resolve_relative)(const AddonInstance_ShaderPreset*
    // addonInstance, video_shader *shader, const char *ref_path); bool (__cdecl*
    // video_shader_resolve_current_parameters)(const AddonInstance_ShaderPreset* addonInstance,
    // video_shader *shader);
    bool(__cdecl* video_shader_resolve_parameters)(const AddonInstance_ShaderPreset* addonInstance,
                                                   preset_file file,
                                                   video_shader* shader);
    void(__cdecl* video_shader_free)(const AddonInstance_ShaderPreset* addonInstance,
                                     video_shader* shader);
...
public:
  CInstanceShaderPreset()
    : IAddonInstance(ADDON_INSTANCE_SHADERPRESET, GetKodiTypeVersion(ADDON_INSTANCE_SHADERPRESET))
  {
    if (CAddonBase::m_interface->globalSingleInstance != nullptr)
      throw std::logic_error("kodi::addon::CInstanceShaderPreset: Creation of more as one in "
  ...
  explicit CInstanceShaderPreset(KODI_HANDLE instance, const std::string& kodiVersion = "")
    : IAddonInstance(ADDON_INSTANCE_SHADERPRESET,
                     !kodiVersion.empty() ? kodiVersion
                                          : GetKodiTypeVersion(ADDON_INSTANCE_SCREENSAVER))
  {
    if (CAddonBase::m_interface->globalSingleInstance != nullptr)
      throw std::logic_error("kodi::addon::CInstanceShaderPreset: Creation of multiple together "
...
private:
  ...
  {
    if (instance == nullptr)
      throw std::logic_error("kodi::addon::CInstanceShaderPreset: Creation with empty addon "
    ...
    // m_instanceData->toAddon.video_shader_resolve_current_parameters =
  ...
  /*
  inline static void ADDON_video_shader_resolve_relative(const AddonInstance_ShaderPreset*
  addonInstance, video_shader *shader, const char *ref_path)
  {
    if (shader != nullptr)
      addonInstance->toAddon.addonInstance->ShaderPresetResolveRelative(*shader, ref_path);
  }

  inline static bool ADDON_video_shader_resolve_current_parameters(const AddonInstance_ShaderPreset*
  addonInstance, preset_file file, video_shader *shader)
  {
    if (shader != nullptr)
      return addonInstance->toAddon.addonInstance->ShaderPresetResolveCurrentParameters(file,
  *shader);
