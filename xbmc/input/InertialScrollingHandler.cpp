...
bool CInertialScrollingHandler::CheckForInertialScrolling(const CAction* action)
{
           ...
           !m_panPoints.empty()) // do we need to animate inertial scrolling?
  {
    ...
    CLog::LogF(LOGDEBUG, "Avg touch velocity: %f,%f up to and including touch at %u ms ago",
               velocityX, velocityY, m_panPoints.front().TimeElapsed());

    if (std::abs(velocityX) > MINIMUM_SPEED_FOR_INERTIA ||
        std::abs(velocityY) > MINIMUM_SPEED_FOR_INERTIA)
    {
      ...
      if (inertialRequested)
      {
        ...
        // CLog::Log(LOGDEBUG, "initial pos: %f,%f velocity: %f,%f dec: %f,%f",
        // m_iLastGesturePoint.x, m_iLastGesturePoint.y, m_iFlickVelocity.x, m_iFlickVelocity.y,
        // m_inertialDeacceleration.x, m_inertialDeacceleration.y);
...
bool CInertialScrollingHandler::ProcessInertialScroll(float frameTime)
{
  ...
  if (m_bScrolling)
  {
    ...
    if (absoluteInertialTime < TIME_TO_ZERO_SPEED)
    {
      ...
      // CLog::Log(LOGDEBUG, "@%f: %f,%f offset: %f, %f velocity: %f,%f dec: %f,%f",
      // absoluteInertialTime,  m_iLastGesturePoint.x, m_iLastGesturePoint.y, xMovement, yMovement,
      // m_iFlickVelocity.x, m_iFlickVelocity.y, m_inertialDeacceleration.x,
      // m_inertialDeacceleration.y); fire the pan action
      g_application.OnAction(CAction(ACTION_GESTURE_PAN, 0, m_iLastGesturePoint.x,
