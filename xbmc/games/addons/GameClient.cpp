...
std::unique_ptr<CGameClient> CGameClient::FromExtension(ADDON::CAddonInfo addonInfo,
                                                        const cp_extension_t* ext)
{
  ...
  for (const auto& property : properties)
  {
    std::string strProperty =
        CServiceBroker::GetAddonMgr().GetExtValue(ext->configuration, property.c_str());
                           ...
                           IGameInputCallback* input)
{
  ...
  try
  {
    LogError(error = m_struct.toAddon.LoadGame(path.c_str()), "LoadGame()");
  }
  catch (...)
  {
...
bool CGameClient::OpenStandalone(RETRO::IStreamManager& streamManager, IGameInputCallback* input)
{
  ...
  try
  {
    LogError(error = m_struct.toAddon.LoadStandalone(), "LoadStandalone()");
  }
  catch (...)
  {
...
bool CGameClient::LoadGameInfo()
{
  ...
  try
  {
    bSuccess = LogError(m_struct.toAddon.GetGameTiming(&timingInfo), "GetGameTiming()");
  }
  catch (...)
  {
  ...
  try
  {
    region = m_struct.toAddon.GetRegion();
  }
  catch (...)
  {
...
void CGameClient::Reset()
{
  ...
  if (m_bIsPlaying)
  {
    try
    {
      LogError(m_struct.toAddon.Reset(), "Reset()");
    }
    catch (...)
    {
      LogException("Reset()");
...
void CGameClient::CloseFile()
{
  ...
  if (m_bIsPlaying)
  {
    ...
    try
    {
      LogError(m_struct.toAddon.UnloadGame(), "UnloadGame()");
    }
    catch (...)
    {
...
void CGameClient::RunFrame()
{
  ...
  if (m_bIsPlaying)
  {
    try
    {
      LogError(m_struct.toAddon.RunFrame(), "RunFrame()");
    }
    catch (...)
    {
...
bool CGameClient::Serialize(uint8_t* data, size_t size)
{
  ...
  if (m_bIsPlaying)
  {
    try
    {
      bSuccess = LogError(m_struct.toAddon.Serialize(data, size), "Serialize()");
    }
    catch (...)
    {
...
bool CGameClient::Deserialize(const uint8_t* data, size_t size)
{
  ...
  if (m_bIsPlaying)
  {
    try
    {
      bSuccess = LogError(m_struct.toAddon.Deserialize(data, size), "Deserialize()");
    }
    catch (...)
    {
