...
const int CDialogGameOSDHelp::CONTROL_OSD_HELP_TEXT = 1101;
const int CDialogGameOSDHelp::CONTROL_OSD_GAME_CONTROLLER = 1102;
...
void CDialogGameOSDHelp::OnInitWindow()
{
  ...
  CGUIMessage msg(GUI_MSG_LABEL_SET, WINDOW_DIALOG_GAME_OSD, CONTROL_OSD_HELP_TEXT);
  ...
  if (CServiceBroker::IsServiceManagerUp())
  {
    ...
    if (controller)
    {
      ...
      CGUIGameController* guiController = dynamic_cast<CGUIGameController*>(m_dialog.GetControl(CONTROL_OSD_GAME_CONTROLLER));
...
{
  return IsVisible(CONTROL_OSD_HELP_TEXT) ||
         IsVisible(CONTROL_OSD_GAME_CONTROLLER);
}
