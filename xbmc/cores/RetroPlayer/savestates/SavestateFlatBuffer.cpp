...
std::string CSavestateFlatBuffer::Caption() const
{
  ...
  if (m_savestate != nullptr && m_savestate->caption())
    caption = m_savestate->caption()->c_str();
...
{
  m_captionOffset.reset(new StringOffset{m_builder->CreateString(caption)});
}
...
void CSavestateFlatBuffer::SetCreated(const CDateTime& createdUTC)
{
  m_createdOffset.reset(
      new StringOffset{m_builder->CreateString(createdUTC.GetAsW3CDateTime(true))});
}
...
bool CSavestateFlatBuffer::Deserialize(std::vector<uint8_t> data)
{
  ...
  if (VerifySavestateBuffer(verifier))
  {
    ...
    {
      CLog::Log(LOGERROR, "RetroPlayer[SAVE): Schema version %u not supported, must be at least version %u",
